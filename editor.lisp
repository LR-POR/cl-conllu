...
(defun apply-action (result-action)
  (let ((field (first result-action)))
    (mapcar
     #'(lambda (token value)
	 (cond ((string= field 'id) (setf (token-id token)
					  (parse-integer value)))
	       ((string= field 'form) (setf (token-form token) value))
	       ((string= field 'lemma) (setf (token-lemma token) value))
	       ((string= field 'upostag) (setf (token-upostag token) value))
	       ((string= field 'feats) (setf (token-feats token) value))
	       ((string= field 'head) (setf (token-head token)
					    (parse-integer value)))
	       ((string= field 'deprel) (setf (token-deprel token) value))
	       ((string= field 'deps) (setf (token-deps token) value))
	       ((string= field 'misc) (setf (token-misc token) value))))
